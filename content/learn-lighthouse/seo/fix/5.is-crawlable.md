...
# Fix Page Blocked from Indexing
...
# Or conditionally for specific paths only
...
**Nginx:**
```nginx
# Remove this from your server block
add_header X-Robots-Tag "noindex";

# Or limit to specific locations
location /admin/ {
  add_header X-Robots-Tag "noindex";
}
```
...
```txt
# Too broad - blocks everything
User-agent: *
Disallow: /

# Better - only block what's needed
User-agent: *
Disallow: /admin/
Disallow: /api/
Disallow: /tmp/
...
```txt
# Accidentally blocking all pages with query strings
Disallow: /*?*

# Blocking all PDFs (maybe unintended)
Disallow: /*.pdf$

# Blocking staging but pattern matches production paths
...
```bash
# Fetch and review your robots.txt
...
# Check robots.txt allows this path
...
- [Robots.txt](/learn-lighthouse/seo/fix/robots-txt) — Check robots.txt for Disallow rules blocking the page
- [HTTP Status Code](/learn-lighthouse/seo/fix/http-status-code) — Both prevent indexing, diagnose which applies
- [Canonical](/learn-lighthouse/seo/fix/canonical) — Noindexed pages shouldn't be canonical targets
