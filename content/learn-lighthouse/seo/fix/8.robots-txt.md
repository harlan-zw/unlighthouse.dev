...
```txt
# Wrong - Disallow without User-agent
Disallow: /private/
...
```txt
# Wrong - pattern doesn't start with / or *
Disallow: admin/
Disallow: private
...
# Supported by some crawlers (use carefully)
Crawl-delay: 10
```
...
- [Is Crawlable](/learn-lighthouse/seo/fix/is-crawlable) — Robots.txt can block pages from indexing
- [HTTP Status Code](/learn-lighthouse/seo/fix/http-status-code) — A 404 robots.txt causes different behavior than missing
- [Canonical](/learn-lighthouse/seo/fix/canonical) — Don't block canonical URLs in robots.txt
