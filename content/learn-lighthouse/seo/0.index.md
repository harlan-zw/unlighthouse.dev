---
title: "Lighthouse SEO Audit: All 8 Checks Explained & How to Fix"
description: "Pass every Lighthouse SEO audit. Crawlability, canonical, meta description, robots.txt, hreflang, status codes — each check explained with fixes and examples."
navigation:
  title: SEO
...
**Lighthouse runs 8 technical SEO checks: viewport meta tag, document title, meta description, HTTP status code, link text, crawlable links, robots.txt validity, and hreflang.** Each audit is pass/fail. A single failure drops your SEO score significantly. This guide covers every check and how to fix it.

Lighthouse SEO audits check whether search engines can find, crawl, and understand your pages. Core Web Vitals measure user experience; SEO audits are binary pass-or-fail checks.

**Important distinction:** [Google doesn't use Lighthouse scores for rankings](https://developers.google.com/search/docs/appearance/page-experience#lighthouse). John Mueller confirmed they use Core Web Vitals data separately from real users (CrUX), not lab scores. Lighthouse SEO audits are diagnostic tools - they help you find problems, but the score itself isn't a ranking factor.
...
[Google's SEO documentation](https://developers.google.com/search/docs/fundamentals/seo-starter-guide) emphasizes that technical SEO is foundational. Technical issues often prevent pages from being indexed or ranking properly - fix those before worrying about content strategy.
...
**Links are crawlable**: Checks for `javascript:void(0)`{lang="ts"}, empty hrefs, or invalid URLs. Impacts PageRank flow and content discovery.
...
- **URL Inspection** tests specific pages, one at a time; the free [bulk index checker](https://gscdump.com/tools/index-checker) checks up to 50 URLs at once
...
[Unlighthouse](/) crawls your entire site and runs SEO audits on every page. You'll see exactly which pages fail each audit, which templates are misconfigured, and whether your fixes work at scale.
