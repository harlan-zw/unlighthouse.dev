...
**Rule of thumb:** If clicking takes you to a new page, use `<a>`{lang="html"} with an href. If it triggers an action on the current page, use `<button>`{lang="html"}.
...
When you generate URLs dynamically, ensure the href is populated at render time - not just on click.
...
If you're using `<a>`{lang="html"} without href as a placeholder or for styling, reconsider.
...
The only valid case for `<a>`{lang="html"} without href is as a named anchor: `<a id="section-1"></a>`{lang="html"}. Even then, prefer `id` on any element.
...
But this is a band-aid. Prefer a real `<button>`{lang="html"} element.

## Framework-Specific Solutions

::callout{icon="i-logos-nextjs-icon"}
**Next.js**: Always use the `Link` component from `next/link`, never raw anchors with click handlers. The Link component renders a proper `<a>`{lang="html"} with href for crawlers.
::
...
- **Using anchors for buttons**: If it doesn't navigate to a URL, it shouldn't be an `<a>`{lang="html"}. Use `<button>`{lang="html"} for actions like modals, dropdowns, and form submissions.

- **Empty href with hash routing**: `<a href="" @click="$router.push('/about')">`{lang="html"} still fails the audit. Use `<router-link>`{lang="html"} or include the href.
