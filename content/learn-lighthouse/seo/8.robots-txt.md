...
**Next.js** - Create `public/robots.txt` for static content, or use `app/robots.ts` for dynamic generation. [Next.js](https://nextjs.org) serves files from `public/` at the root path automatically. For dynamic robots.txt based on environment, export a `robots()`{lang="ts"} function from `app/robots.ts`.
::
...
2. Check Search Console's [robots.txt report](https://support.google.com/webmasters/answer/6062598) (Settings > robots.txt) for fetch and parse errors. It replaced the retired robots.txt Tester
