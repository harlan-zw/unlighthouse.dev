...
- Must be in the `<head>`{lang="html"}, not `<body>`{lang="html"}
...
Or with Nuxt SEO module, it handles canonicals automatically based on your site URL configuration.
::
...
Check that exactly one `<link rel="canonical">`{lang="html"} appears in `<head>`{lang="html"} with a valid absolute URL.
...
Use URL Inspection to see which canonical Google detected. If it differs from yours, Google is overriding your choice - usually because something else signals a different preferred URL. To compare the Google-selected canonical across many URLs at once, the free [bulk index checker](https://gscdump.com/tools/index-checker) reads it from the same URL Inspection API for up to 50 URLs.
...
- **Canonical in body**: Canonicals in `<body>`{lang="html"} are ignored by search engines. Must be in `<head>`{lang="html"}.
