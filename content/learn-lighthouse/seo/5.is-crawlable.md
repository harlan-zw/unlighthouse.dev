...
3. Look for `<meta name="robots">`{lang="html"} with `noindex` or `none` in the content
...
- `<meta name="robots">`{lang="html"} for `noindex` or `none` directives
...
Check Search Console's [robots.txt report](https://support.google.com/webmasters/answer/6062598) for the file Google fetched and any parse errors. To check that a specific URL is allowed, use URL Inspection.
