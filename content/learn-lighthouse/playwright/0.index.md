...
Run Lighthouse audits programmatically using [Playwright](https://playwright.dev). This guide uses the Lighthouse Node API directly, so you control the browser, audit settings, and report output.

::note
**Requirements**: [Node.js](https://nodejs.org) 22.19 or newer, Playwright installed. Lighthouse connects via Chrome DevTools Protocol on a remote debugging port.
::
...
Both tools drive Chromium through CDP, so either can host a Lighthouse audit. Use Playwright when the audit belongs in an existing Playwright test suite. Lighthouse's own authenticated-page documentation uses Puppeteer, which is a good fit for a standalone audit script.

| Feature | Playwright | Puppeteer |
|---------|------------|-----------|
| Test runner | Built in (`@playwright/test`) | Bring your own (Jest, Mocha) |
| Auth setup | `storageState` and fixtures | Scripted browser profile or cookies |
| Parallel workers | Built into Playwright Test | Bring your own runner |
...
Lighthouse 13 requires Node.js 22.19 or newer and ships as ESM. Current [Playwright installation guidance](https://playwright.dev/docs/intro) supports maintained Node.js 22, 24, and 26 releases. Use an ESM test project and keep Playwright's browser install in sync with the package version recorded by your lockfile.
...
Lighthouse cannot measure INP in a synthetic page load because INP needs real interactions across a page visit. Keep [Total Blocking Time](/glossary/tbt) as the lab diagnostic, then verify [INP](/glossary/inp) with CrUX or your own real-user monitoring.

```ts
const tbt = lhr.audits['total-blocking-time']?.numericValue
console.log(`TBT: ${tbt ?? 'unavailable'}ms`)
```

Do not compare scores across a Lighthouse or Chromium upgrade as if the test environment stayed fixed. Record tool versions with each CI result, then establish a new baseline after upgrading.
...
[Export the Lighthouse report](/guide/guides/generating-static-reports), then use `testInfo.attach()`{lang="ts"} to include HTML and JSON output in the Playwright HTML report. Attached files appear under each test result:
