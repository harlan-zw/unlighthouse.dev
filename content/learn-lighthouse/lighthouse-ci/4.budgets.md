---
title: "Performance Budgets with Lighthouse CI: Assertions & budget.json"
description: "Set and enforce performance budgets with Lighthouse CI. Configure assertions, presets, custom thresholds, and budget.json to prevent regressions in CI/CD."
navigation:
...
| Finding | Source |
|---------|--------|
| **0.1s improvement = 8.4% retail conversion increase** | [Google's research on the business impact of Core Web Vitals](https://web.dev/case-studies/vitals-business-impact) |
| **Every 100ms costs Amazon 1% in sales** | [Cloudflare](https://www.cloudflare.com/en-au/learning/performance/more/website-performance-conversion-rates/) |
| **BBC loses 10% of users per additional second** | [Google Research](https://web.dev/learn/performance/why-speed-matters) |
| **Bounce probability increases 90% as load goes 1s→5s** | [Google Research](https://web.dev/learn/performance/why-speed-matters) |
| **Good LCP = 61% conversion increase for Rakuten** | [web.dev Case Study](https://web.dev/case-studies/rakuten) |

### The Page Weight Problem

[Page weight has grown significantly over the last decade](https://almanac.httparchive.org/en/2024/page-weight) (484KB → 2.2MB average). The [2024 Web Almanac](https://almanac.httparchive.org/en/2024/page-weight) reports median page weight at 2,675 KB (+8% YoY) with median JavaScript at 558 KB on mobile.

[Alex Russell's research](https://almanac.httparchive.org/en/2024/performance) suggests a **365 KB JavaScript budget** for sub-3-second loads on typical mobile devices - yet the 75th percentile exceeds 650 KB. Before setting budgets, measure your current baseline with the [Page Size Checker](/tools/page-size).
...
Strictest - makes sure worst case meets budget. Good for critical paths.
...
Checks that metric correlations are preserved. Use when metrics interrelate.
...
::warning
**Lab scores ≠ field performance**: [Analysis of real-world CrUX data](https://web.dev/articles/lab-and-field-data-differences) found that a significant portion of pages scoring high in Lighthouse may still fail one or more Core Web Vitals thresholds in real-world conditions. Use [RUM monitoring](/learn-lighthouse/core-web-vitals) alongside synthetic testing.
::
...
Binary pass/fail audits don't need thresholds. See the [accessibility guide](/learn-lighthouse/accessibility) for complete audit details.
