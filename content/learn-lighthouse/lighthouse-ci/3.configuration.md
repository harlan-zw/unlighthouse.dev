...
Lighthouse CI uses configuration files to define how it collects audits, evaluates assertions, and uploads results.
...
Choose how Lighthouse CI combines multiple runs:
...
::note{type="info"}
**Why median?** Since the performance score is already a weighted average, taking the mean of scores compounds outlier effects. [Lighthouse CI calculates median](https://github.com/GoogleChrome/lighthouse/blob/main/core/lib/median-run.js) by finding the run closest to median FCP **and** median TTI (earliest + latest page lifecycle moments), not just the score.
::

::warning
**Never run concurrent Lighthouse audits** on the same machine. [Concurrent runs skew results](https://github.com/GoogleChrome/lighthouse/blob/main/docs/variability.md) due to resource contention.
::
...
The `upload` configuration controls where Lighthouse CI stores audit results.
...
Strict production budgets require optimization across all [Core Web Vitals](/learn-lighthouse/core-web-vitals). Target values: [LCP](/learn-lighthouse/lcp) under 2.5s, [CLS](/learn-lighthouse/cls) under 0.1, [TBT](/glossary/tbt) under 200ms.
...
- [Performance Budgets](/learn-lighthouse/lighthouse-ci/budgets) - Deep look at assertion strategies
...
- [GitLab CI](/learn-lighthouse/lighthouse-ci/gitlab-ci) - [GitLab](https://gitlab.com) integration
