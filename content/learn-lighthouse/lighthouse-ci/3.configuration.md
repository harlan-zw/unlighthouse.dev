...
::note{type="info"}
**Why median?** Since the performance score is already a weighted average, taking the mean of scores compounds outlier effects. [Lighthouse CI calculates median](https://maxrozen.com/how-to-choose-median-lighthouse-multiple-runs) by finding the run closest to median FCP **and** median TTI (earliest + latest page lifecycle moments), not just the score.
::

::warning
**Never run concurrent Lighthouse audits** on the same machine. [Concurrent runs skew results](https://www.debugbear.com/docs/reduce-lighthouse-variance) due to resource contention.
::
