...
If using an older Node version, you may need to upgrade [Node.js](https://nodejs.org) to match the Chrome version installed on your system.
...
**Problem:** `Browser tab has unexpectedly crashed` or `TARGET_CRASHED` errors in [Docker](https://docker.com)
...
2. **Test against origin**: bypass CDN for CI testing
...
::warning
[Skip rules don't work for Super Bot Fight Mode](https://community.cloudflare.com/t/turn-off-bot-detection-for-specific-url/565739). You must disable SBFM or test a different endpoint.
::
...
- Use [simulated throttling](https://github.com/GoogleChrome/lighthouse/blob/master/docs/throttling.md) (default) for lowest variance
...
This makes sure historical data aggregates properly across different deployments.
...
**Solution**: Check that file is in project root and properly named:
...
- [Vite](https://vite.dev): `Local:.*http://localhost`
- [Next.js](https://nextjs.org): `ready on`
...
- **Fix the issues:** Look at [LCP](/learn-lighthouse/lcp), [CLS](/learn-lighthouse/cls), and [INP](/learn-lighthouse/inp) optimization guides
