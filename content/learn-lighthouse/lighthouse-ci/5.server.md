...
- Database: [SQLite](https://sqlite.org), [MySQL](https://mysql.com), or [PostgreSQL](https://www.postgresql.org/)
...
Pre-built [Docker](https://docker.com) image makes deployment straightforward.
...
Heroku automatically sets `DATABASE_URL` for [PostgreSQL](https://postgresql.org) connection.
...
Best for single-server deployments, testing, and low traffic. [Works well for small teams](https://github.com/GoogleChrome/lighthouse-ci/blob/main/docs/server.md) but gets bigger and slower over time.
...
::warning
**SQLite limitations**: Not optimized for high write volumes. [Azure storage file shares are incompatible with SQLite](https://learn.microsoft.com/en-us/azure/app-service/configure-connect-to-azure-storage#best-practices) - use PostgreSQL for cloud deployments.
::
...
Recommended for production deployments. [Better query performance](https://www.sqlite.org/whentouse.html) and handles concurrent writes from multiple CI pipelines.
