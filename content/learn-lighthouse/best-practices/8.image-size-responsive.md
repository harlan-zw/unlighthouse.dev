...
## Mobile-First Indexing & 2025 Targets

Google's 2025 mobile-first indexing standards have tightened. Images are a primary driver of mobile bandwidth usage.

- **Network Request Target:** Aim for **<50 total network requests** on mobile. Consolidating images or lazy-loading them is critical.
- **CLS Impact:** Undersized or un-dimensioned images are the #1 cause of Cumulative Layout Shift (CLS). A CLS score > 0.1 will penalize your mobile rankings.
...
2. In Elements panel, look at the `<img>`{lang="html"} element
...
The direct fix: make sure your image files are large enough.
...
- **CSS background images** - This audit only covers `<img>`{lang="html"} elements. Background images need manual srcset via `image-set()`{lang="ts"}.
