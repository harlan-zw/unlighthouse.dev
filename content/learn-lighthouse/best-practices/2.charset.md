...
The `<meta charset>`{lang="html"} must appear before any content that contains characters - ideally as the first element after `<head>`{lang="html"}.
...
Place the charset declaration as the first element in `<head>`{lang="html"}:
...
Configure your server to send the charset in the Content-Type header. The browser processes this before parsing the HTML, providing an additional layer of reliability.
...
If you have lots of content before your charset (long inline scripts, many meta tags), restructure your `<head>`{lang="html"}:
...
- **Using http-equiv instead of charset**: The legacy format `<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">`{lang="html"} works but is verbose. Use `<meta charset="UTF-8">`{lang="html"} for HTML5.

- **Placing charset after title or other meta**: The charset must be early enough for the browser to read it before it starts parsing text content. Put it first in `<head>`{lang="html"}.
...
- **Mixing encodings**: If your file is saved as ISO-8859-1 but you declare UTF-8, characters will break. Ensure your editor saves files as UTF-8 (check encoding settings in [VS Code](https://code.visualstudio.com), Sublime, etc.).

- **Large inline scripts before charset**: The 1024-byte limit is firm. Move large inline scripts to external files or after other critical `<head>`{lang="html"} elements.

## Test Your Entire Site

Different templates may have different `<head>`{lang="html"} structures. Scan your entire site to ensure all pages declare charset correctly.
