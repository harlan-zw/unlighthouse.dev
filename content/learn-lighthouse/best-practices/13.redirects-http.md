...
While not strictly a redirect issue, "Soft 404s" (pages that say "Not Found" but return a `200 OK` status) are a primary waste of crawl budget in 2025. Google's bots now aggressively deprioritize sites with poor status code integrity. Ensure your 404 pages return a `404` status header.
