...
tags:
  ...
  - ux
relatedPages:
  - path: /learn-lighthouse/best-practices/geolocation-on-start
...
Your code calls `Notification.requestPermission()`{lang="ts"} during page load - before the user has expressed any interest in receiving notifications. The browser shows a permission prompt, the user blocks it reflexively, and that decision persists until they manually dig through browser settings to reverse it.

Chrome has started hiding these prompts for sites that abuse them. Your notification request might not even be shown anymore if your site has a poor grant rate.
...
- **Using a short delay** - `setTimeout(() => Notification.requestPermission(), 5000)`{lang="ts"} still runs automatically. Five seconds isn't user engagement; it's slower annoyance.
