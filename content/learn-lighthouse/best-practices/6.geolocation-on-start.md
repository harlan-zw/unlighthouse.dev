...
tags:
  ...
  - ux
relatedPages:
  - path: /learn-lighthouse/best-practices/notification-on-start
...
Your JavaScript calls `navigator.geolocation.getCurrentPosition()`{lang="ts"} or `navigator.geolocation.watchPosition()`{lang="ts"} during page load - before the user has done anything that suggests they want location features. The browser shows a permission prompt immediately, the user reflexively clicks "Block," and now your location features are permanently broken for that visitor.
...
- **Using setTimeout to delay** - `setTimeout(() => { geolocation... }, 5000)`{lang="ts"} is still automatic. Lighthouse may not catch it, but users will still see an unexplained prompt.

- **Checking permissions then requesting** - Even `navigator.permissions.query()`{lang="ts"} followed by an automatic request is wrong. The permission check is fine; the automatic request isn't.
