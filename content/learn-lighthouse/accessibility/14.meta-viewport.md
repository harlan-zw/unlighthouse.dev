...
- **Mobile-first indexing**: Without a proper viewport tag, mobile browsers render the page as a shrunken desktop site. Google indexes with a smartphone crawler, so that broken mobile render is the version it evaluates. Google [retired its Mobile-Friendly Test](https://developers.google.com/search/blog/2016/05/a-new-mobile-friendly-testing-tool) on 1 December 2023, so the Lighthouse viewport audit is now the quickest check.
