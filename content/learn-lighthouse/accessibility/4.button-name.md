---
title: Fix Buttons Without Accessible Names
...
When a button lacks an accessible name, screen readers announce it as "button" with no indication of what it does. Users have to guess if it submits a form, opens a menu, or deletes data.

## What's happening

Lighthouse flags `<button>`{lang="html"} elements and elements with `role="button"` that have no discernible text. The audit checks for visible text content, `aria-label`, `aria-labelledby`, or `title` attributes. Buttons containing only icons, images without alt text, or empty content will fail this audit.
...
5. Open the Accessibility tab to view the computed accessible name (it must not be empty).

## Fix

### 1. Add visible button text

Use visible text. It's the most straightforward fix.
