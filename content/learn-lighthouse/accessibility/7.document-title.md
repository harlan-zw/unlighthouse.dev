...
The `<title>`{lang="html"} element provides the page's name that appears in browser tabs, bookmarks, and search results. Screen readers announce the title when users arrive at a page, giving immediate context.
...
1. Open DevTools (F12).
2. Elements panel > search for `<title>`{lang="html"}.
3. Or run in Console:
...
2. Look for `<title>`{lang="html"} within `<head>`{lang="html"}.
...
Add `<title>`{lang="html"} inside `<head>`{lang="html"} on every page:
...
To find the titles worth rewriting first, the free [CTR opportunity finder](https://gscdump.com/tools/opportunity) lists the queries in your Search Console data whose click-through rate sits below the benchmark for their position.

## Verify the fix

1. **Re-run Lighthouse** - The "Document has a `<title>`{lang="html"} element" audit should pass.
...
- **Empty title tag**: `<title></title>`{lang="html"} passes the "exists" check but fails accessibility. The title must contain text.

- **Same title on all pages**: `<title>Home</title>`{lang="html"} on every page makes navigation impossible. Each page needs a unique title.
...
- **Special characters not encoded**: Encode ampersands and other special characters: `<title>Tom &amp; Jerry</title>`{lang="html"}
