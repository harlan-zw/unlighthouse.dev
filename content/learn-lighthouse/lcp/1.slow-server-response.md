...
Time to First Byte (TTFB) measures the duration between the browser requesting a page and receiving the first byte of the response. Lighthouse flags this as "Reduce initial server response time" when your document takes longer than **600ms** to respond.

The real-world thresholds for Core Web Vitals are: **Good under 800ms**, Needs Improvement between 800-1800ms, and Poor above 1800ms. However, for a high Lighthouse score, you should aim for **under 600ms**.
...
5. Anything above **600ms** will trigger a Lighthouse audit failure; above **800ms** fails the Core Web Vitals "Good" threshold.
...
Lighthouse reports "Reduce initial server response time" under Diagnostics when the root document exceeds the **600ms** threshold. The audit shows the actual response time and calculates potential savings toward FCP and LCP metrics.
...
**Next.js**: Use App Router with automatic streaming. Deploy to [Vercel](https://vercel.com) for built-in edge caching. Configure `generateStaticParams` for static generation. Add ISR with `revalidate` for dynamic content that changes infrequently.
...
**Nuxt**: Configure `routeRules` for per-route caching strategies. Use `prerenderRoutes` for static paths. Deploy to [Cloudflare](https://cloudflare.com) or Vercel for edge rendering. Enable `experimental.componentIslands` for partial hydration.
