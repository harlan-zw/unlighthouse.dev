...
- Blue/green bars = used bytes (executed)
...
Parse and compile time particularly impact mobile devices. V8 (Chrome's engine) can parse JavaScript at roughly 1MB/second on a mid-range phone. A 500KB bundle takes 500ms of main thread time to parse - before any code executes.
...
[Next.js](https://nextjs.org) automatically code-splits by route. For component-level splitting:
...
Vue with [Vite](https://vite.dev) handles async components natively:
...
2. Open Coverage tab → you should see significantly reduced red bars
...
- **Ignoring third-party bundles**: Heavy [npm](https://npmjs.com) packages often need replacement, not just splitting
