---
title: "Eliminate Render-Blocking Resources: Lighthouse Audit Fix"
description: "How to eliminate render-blocking CSS and JavaScript in the Lighthouse audit. Async/defer scripts, critical CSS extraction, font loading, and Next.js optimizeCss config."
navigation:
...
Lighthouse flags this as "Eliminate render-blocking resources" and calculates the potential time savings. The audit identifies CSS files loaded in the `<head>`{lang="html"} without `media` attributes and JavaScript files without `async` or `defer`. Each blocking resource in the chain extends the time before First Contentful Paint and LCP.
...
Never load JavaScript synchronously in the `<head>`{lang="html"} unless necessary.
...
**Nuxt**: Enable inline SSR styles for automatic critical CSS. Use component-level styles with `<style scoped>`{lang="html"}. Configure font loading with `@nuxt/fonts` for optimal font-display settings.
...
Different page templates have different blocking resources. Your blog might inline critical CSS while your product pages load three external stylesheets. Run a complete audit to find render-blocking issues across every template.
