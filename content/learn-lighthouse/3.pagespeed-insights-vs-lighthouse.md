...
A 10-15 point gap between PSI and local Lighthouse is normal. [Score variance of ±5 points](https://support.nitropack.io/en/articles/8390477-pagespeed-insights-test-results-vary-with-a-few-points) happens even across identical PSI runs. Here's what causes larger gaps:
...
PSI routes your test to whichever Google data center is closest to your IP - not closest to your server. [As research on PSI server locations shows](https://www.debugbear.com/blog/pagespeed-insights-server-locations), if your site lacks a CDN, a test routed from Taiwan to a US-only origin adds hundreds of milliseconds to TTFB, inflating LCP.
...
Google's servers have different CPU/RAM profiles than your laptop. Simulated throttling tries to normalize this, but CPU-heavy pages (lots of JavaScript parsing) can score differently depending on the basic hardware.
...
**Lighthouse cannot measure INP.** It uses Total Blocking Time (TBT) as a lab proxy, but TBT only measures main-thread blockage **during initial load**. A page can achieve 0ms TBT and still fail INP in the field because JavaScript freezes the page when users interact after load. [Read more about the INP vs TBT gap](https://greenpagespeed.co.uk/articles/inp-vs-tbt/).
::
...
PSI runs Lighthouse on Google's servers with different hardware, network conditions, and server locations than your local machine. PSI also overlays CrUX field data that can affect the reported metrics. A gap of [±5-15 points](https://web.dev/articles/performance-scoring) between the two is normal.
...
Optimize for real users. Use Lighthouse to diagnose and fix lab issues during development. Use PSI field data to verify improvements reach your actual audience. Don't chase a perfect 100 - [diminishing returns hit hard above 90](https://web.dev/articles/performance-scoring).
