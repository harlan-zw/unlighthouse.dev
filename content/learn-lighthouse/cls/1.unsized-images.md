...
1. Browser parses HTML, finds `<img>`{lang="html"} with no dimensions
...
The simplest fix. Add `width` and `height` to every `<img>`{lang="html"} element:
...
The insight: you're not preventing the image from loading - you're preventing the layout from changing when it does.
...
The component generates `srcset` and handles responsive sizing. Using raw `<img>`{lang="html"} in [Next.js](https://nextjs.org) usually means missing out on these protections.
::
...
Lighthouse requires both dimensions to calculate the aspect ratio correctly.
