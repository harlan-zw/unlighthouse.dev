...
2. Right-click `<body>`{lang="html"} > Break on > Subtree modifications
...
It's possible to have a "Good" CLS score for real users but a "Poor" score in Google Search Console. This often happens due to **bot rendering differences**:

1. **Robots.txt Blocking**: If your `robots.txt` blocks Googlebot from crawling specific CSS or JS files used for layout (e.g., a styling script for a dynamic banner), the bot renders the page without them. It might see a layout shift that real users don't.
2. **Cookie Banners**: Bots visit as "new users" every time. They *always* see the cookie banner. If you only test as a logged-in user or with cookies accepted, you'll miss the massive shift the bot sees on every crawl.
3. **Viewport Meta Tag**: Ensure `<meta name="viewport" content="width=device-width, initial-scale=1">`{lang="html"} is present. Without it, mobile bots may render a desktop width and then snap to mobile, triggering shifts.
