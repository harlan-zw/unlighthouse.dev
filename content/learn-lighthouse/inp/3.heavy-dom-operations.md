...
Your browser spent 4 seconds doing work on the main thread. During that time? Every user interaction waited in line. The `mainthread-work-breakdown` audit measures everything the browser does: parsing, compiling, layout, paint, JavaScript execution. When this total exceeds 2 seconds, interactions suffer.
...
Common properties that trigger layout: `offsetWidth`, `offsetHeight`, `offsetTop`, `offsetLeft`, `scrollTop`, `scrollHeight`, `clientWidth`, `clientHeight`, `getComputedStyle()`{lang="ts"}, `getBoundingClientRect()`{lang="ts"}.
...
This makes sure updates happen once per frame at most, not dozens of times per scroll event.
...
- `filter: blur()`{lang="ts"} on large elements
...
Use `computed` properties which Vue caches:
...
**Over-using will-change.** Adding `will-change: transform` to everything consumes memory. Use it sparingly on elements that animate.
