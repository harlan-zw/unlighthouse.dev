...
relatedPages:
  - path: /learn-lighthouse/inp
    title: INP Overview
  - path: /learn-lighthouse/inp#common-inp-issues
    title: All INP Issues
  - path: /glossary/inp
    title: INP Definition
  - path: /tools/inp-analyzer
    title: INP Analyzer Tool
---
...
**Unoptimized framework hydration.** [React](https://react.dev), Vue, and [Svelte](https://svelte.dev) all need to "wake up" server-rendered HTML. Poorly structured component trees mean thousands of elements traversed synchronously.
...
**Why `scheduler.yield()`{lang="ts"} over `setTimeout(0)`{lang="ts"}:** Both yield control, but `setTimeout` moves your task to the back of the queue. Other scripts can cut in line. `scheduler.yield()`{lang="ts"} maintains your position while still letting the browser handle urgent work like user input.
...
The `isInputPending()`{lang="ts"} API lets you yield only when users are waiting.
...
Use `dynamic()`{lang="ts"} for components not needed at initial render:
...
Use `callOnce()`{lang="ts"} to prevent duplicate initialization:
...
**Using setTimeout for everything.** `setTimeout(fn, 0)`{lang="ts"} still blocks if the callback runs too long. You need to yield inside the callback, not before it.
