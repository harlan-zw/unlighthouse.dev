...
The `requestAnimationFrame` + `setTimeout` pattern makes sure the browser paints before heavy work continues.

### Use `scheduler.yield()`{lang="ts"} to break up work
...
[React](https://react.dev) 18's concurrent features let urgent updates interrupt non-urgent ones.
::
...
For expensive computed properties, consider `computedAsync` from [VueUse](https://vueuse.org) or moving logic to a worker.
::
