...
| Interactions | None (task timing) | Clicks, taps, keypresses |
...
Use `<ClientOnly>`{lang="html"} to defer non-critical hydration:
...
Use `React.lazy()`{lang="ts"} for route-level code splitting:
