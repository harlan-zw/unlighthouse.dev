...
<template>
  <div class="rounded-lg border border-default overflow-hidden bg-default">
    <!-- Browser chrome -->
    <div class="flex items-center gap-2 px-3 py-2 bg-accented border-b border-default">
      ...
      <div class="flex-1 mx-2">
        <div class="flex items-center gap-2 bg-default rounded-md px-3 py-1 text-xs text-muted font-mono">
      ...
      <div v-if="status" class="flex items-center gap-1.5 text-xs">
        ...
        />
        <span class="text-muted">{{ status === 'loading' ? 'Loading' : status === 'complete' ? 'Loaded' : 'Ready' }}</span>
      </div>
    </div>
