...
const { syncParam } = useToolUrlSync(urlInput, {
  extraParams: { mode: mode as Ref<string>, device },
  onReady: () => lookup(),
})
...
<template>
  <div class="min-h-screen">
    ...
    <ToolPageHero
      title="TTFB"
      accent="Checker"
      description="Test Time to First Byte with real Chrome user data and lab measurements."
      color="sky"
    />

    <ToolCard icon="i-heroicons-clock" title="Time to First Byte" subtitle="Server response time" color="sky">
      <!-- Input -->
      <div class="p-4 sm:p-6 border-b border-gray-200 dark:border-gray-800">
        <form class="space-y-4" @submit.prevent="lookup">
          ...
          <div class="flex flex-col sm:flex-row flex-wrap gap-3 items-stretch sm:items-center sm:justify-between">
            ...
            <ToolDeviceToggle v-model="device" :disabled="loading" />
      ...
      <ToolLoadingPill v-if="loading" :message="loadingMessage" color="sky" hint="Looking up real user data from CrUX." />
