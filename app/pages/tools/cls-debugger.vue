...
<template>
  <div class="min-h-screen">
    <ToolPageHero
      title="CLS"
      accent="Debugger"
      description="Find which elements cause layout shifts and fix the jank."
      color="amber"
    />

    <ToolCard icon="i-heroicons-arrows-pointing-out" title="CLS Analysis" color="amber" max-width="max-w-5xl">
      <!-- Input -->
      ...
      <div v-if="loading" ref="loadingContainerRef">
        <ToolLoadingPill :message="loadingMessage" color="amber" hint="This can take up to 2 minutes. The API runs a full Lighthouse audit." />
      </div>
      ...
      <!-- Empty State -->
      <ToolEmptyState v-if="!result && !loading && !error" icon="i-heroicons-arrows-pointing-out" message="Enter a URL to debug layout shifts" />
