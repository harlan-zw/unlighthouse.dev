...
// Insights
const insights = computed<ToolInsight[]>(() => {
  if (!result.value)
    return []

  const items: ToolInsight[] = []
...
<template>
  <div class="min-h-screen">
    ...
    <ToolPageHero
      title="INP"
      accent="Analyzer"
      description="Find what's blocking your main thread and slowing down interactions."
      color="cyan"
    />

    <ToolCard icon="i-heroicons-cursor-arrow-rays" title="INP Analysis" color="cyan" max-width="max-w-5xl">
      <!-- Input -->
      ...
      <!-- Loading State -->
      <ToolLoadingPill v-if="loading" :message="loadingMessage || 'Analyzing...'" color="cyan" hint="This can take up to 2 minutes. The API runs a full Lighthouse audit." />

      <!-- Error -->
      <ToolError :error="error" />

      <!-- Empty State -->
      <ToolEmptyState v-if="!result && !loading && !error" icon="i-heroicons-cursor-arrow-rays" message="Enter a URL to analyze INP" />
      ...
      <div v-if="result" class="p-4 sm:p-6">
        ...
        <ToolInsightList :insights="insights" />
