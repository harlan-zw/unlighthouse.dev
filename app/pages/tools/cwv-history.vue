...
const { syncParam } = useToolUrlSync(urlInput, {
  extraParams: {
    mode: mode as Ref<string>,
    device: deviceToggle as unknown as Ref<string>,
  },
  onReady: () => lookup(),
})
...
function getRating(rating: CruxRating | null | undefined) {
  return rating ? ratingColors[rating] : nullRating
}
...
<template>
  <div class="min-h-screen">
    ...
    <ToolPageHero
      ...
      accent="History"
      description="Track Core Web Vitals trends over time with 25 weeks of real Chrome user data."
      color="indigo"
    />

    <ToolCard icon="i-heroicons-chart-bar" title="CrUX Historical Data" subtitle="Field data from real Chrome users" color="indigo">
      <!-- Input -->
        ...
        <form class="space-y-4" @submit.prevent="lookup">
          ...
          <div class="flex flex-col sm:flex-row flex-wrap gap-3 items-stretch sm:items-center sm:justify-between">
            ...
            <div class="flex items-center gap-2 w-full sm:w-auto">
              ...
              <ToolDeviceToggle v-model="deviceToggle" :disabled="loading" />
            </div>
      ...
      <ToolLoadingPill v-if="loading" :message="loadingMessage" color="indigo" hint="Fetching 25 weeks of real user data." />
        ...
        <div>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div
              ...
              :class="[
                getRating(metric.rating).bg,
                getRating(metric.rating).border,
            ...
            >
              ...
              <div class="text-3xl font-bold mb-2" :class="getRating(metric.rating).text">
                {{ metric.displayValue }}
              </div>
              <UIcon
                v-if="metric.rating"
                ...
                :class="getRating(metric.rating).text"
              />
              <div class="text-xs font-medium uppercase" :class="getRating(metric.rating).text">
        ...
        <div v-if="supportingMetrics.some(m => m.value !== undefined)">
          ...
          <div class="grid grid-cols-2 gap-3">
            ...
            >
              ...
              <div class="text-xl font-bold" :class="getRating(metric.rating).text">
                {{ metric.displayValue }}
              </div>
