...
import { useToolLighthouseReport } from '../../composables/useToolLighthouseReport'
...
const { report, error, loading, loadFromFile, loadFromText, clear } = useToolLighthouseReport()

const { trackUse } = useToolTracking('lighthouse-report-viewer')
...
<template>
  <div class="min-h-screen">
    <ToolPageHero title="Lighthouse Report" accent="Viewer" description="Upload or paste a Lighthouse JSON report to analyze scores, Core Web Vitals, and audits interactively." color="violet" />

    <ToolCard icon="i-heroicons-document-chart-bar" title="Report Viewer" color="violet" max-width="max-w-5xl">
      ...
      <ToolLoadingPill v-if="loading" message="Parsing report..." color="violet" />
      ...
      <div v-if="report" class="p-4 sm:p-6 space-y-6">
        ...
        <div>
          ...
          <ToolCategoryScores :categories="categoryDisplayData" />
        </div>
        ...
        <div v-if="report.performanceMetrics.length">
          ...
          <ToolMetricsPanel :metrics="report.performanceMetrics" />
        </div>
    ...
    <section class="px-3 sm:px-6 lg:px-8 pb-12">
      <div class="max-w-6xl mx-auto">
        ...
        <!-- Feedback -->
        <ToolFeedback tool-id="lighthouse-report-viewer" :ready="!!report" :context="{ hasReport: !!report }" />
