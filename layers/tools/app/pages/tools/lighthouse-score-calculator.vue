...
const calc = useToolLighthouseCalculator()
...
function fetchFromUrl() {
  ...
  $fetch('/api/tools/pagespeed-insights', {
    query: { url, strategy: calc.device.value },
  })
    .then((result: Record<string, any>) => {
      for (const metric of result.metrics) {
        const calcId = PSI_METRIC_MAP[metric.id]
        if (calcId)
          calc.setMetricValue(calcId, metric.numericValue)
      }
      ...
      <div class="max-w-5xl mx-auto">
        <div class="relative">
          ...
          <div class="relative mb-3">
            <form class="flex items-center gap-2" @submit.prevent="fetchFromUrl">
              <div class="relative flex-1">
                <UIcon name="i-heroicons-globe-alt" class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400 pointer-events-none" />
                <input
                  v-model="urlInput"
                  type="url"
                  placeholder="Paste a URL to load real PageSpeed metrics..."
                  class="w-full pl-9 pr-3 py-2.5 rounded-xl bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 text-sm placeholder:text-gray-400 dark:placeholder:text-gray-500 focus:outline-none focus:ring-2 focus:ring-violet-500/40 focus:border-violet-500 transition-all"
              ...
              <UButton
                ...
                class="shrink-0"
              >
                <UIcon v-if="!fetchLoading" name="i-heroicons-play-solid" class="w-3.5 h-3.5 mr-1" />
                {{ fetchLoading ? 'Testing...' : 'Test URL' }}
              </UButton>
            </form>
            ...
            >
              <p v-if="fetchLoading" class="mt-2 text-xs text-violet-600 dark:text-violet-400 pl-1">
                {{ loadingMessage }}
              </p>
            </Transition>

            <!-- Error -->
            <p v-if="fetchError" class="mt-2 flex items-center gap-1.5 text-xs text-red-500 pl-1">
